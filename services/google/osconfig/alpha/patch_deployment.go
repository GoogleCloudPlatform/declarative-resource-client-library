...
type PatchDeploymentPatchConfig struct {
	...
	RetryStrategy *PatchDeploymentPatchConfigRetryStrategy    `json:"retryStrategy"`
}
...
func (r *PatchDeploymentPatchConfig) UnmarshalJSON(data []byte) error {
	...
	if len(m) == 0 {
		*r = *EmptyPatchDeploymentPatchConfig
		...
		r.RetryStrategy = res.RetryStrategy
...
type PatchDeploymentPatchConfigRetryStrategy struct {
	empty   bool  `json:"-"`
	Enabled *bool `json:"enabled"`
}

type jsonPatchDeploymentPatchConfigRetryStrategy PatchDeploymentPatchConfigRetryStrategy

func (r *PatchDeploymentPatchConfigRetryStrategy) UnmarshalJSON(data []byte) error {
	var res jsonPatchDeploymentPatchConfigRetryStrategy
	if err := json.Unmarshal(data, &res); err != nil {
		return err
	}
	...
	if len(m) == 0 {
		*r = *EmptyPatchDeploymentPatchConfigRetryStrategy
	} else {
...
// This object is used to assert a desired state where this PatchDeploymentPatchConfigRetryStrategy is
// empty. Go lacks global const objects, but this object should be treated
// as one. Modifying this object will have undesirable results.
