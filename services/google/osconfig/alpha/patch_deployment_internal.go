...
func comparePatchDeploymentPatchConfigRetryStrategyNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Enabled, actual.Enabled, dcl.DiffInfo{OperationSelector: dcl.TriggersOperation("updatePatchDeploymentUpdatePatchDeploymentOperation")}, fn.AddNest("Enabled")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func extractPatchDeploymentPatchConfigFields(r *PatchDeployment, o *PatchDeploymentPatchConfig) error {
	...
	if !dcl.IsEmptyValueIndirect(vRetryStrategy) {
