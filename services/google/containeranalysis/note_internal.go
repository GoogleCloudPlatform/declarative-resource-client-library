...
func (r *Note) validate() error {
	...
	if err := dcl.ValidateExactlyOneOfFieldsSet([]string{"Vulnerability", "Build", "Package", "Discovery"}, r.Vulnerability, r.Build, r.Package, r.Discovery); err != nil {
		return err
	}
...
func canonicalizeNoteDesiredState(rawDesired, rawInitial *Note, opts ...dcl.ApplyOption) (*Note, error) {
	...
	if dcl.IsZeroValue(rawDesired.Vulnerability) {
		// check if anything else is set
		if dcl.AnySet(rawDesired.Build, rawDesired.Package, rawDesired.Discovery) {
	...
	if dcl.IsZeroValue(rawDesired.Build) {
		// check if anything else is set
		if dcl.AnySet(rawDesired.Vulnerability, rawDesired.Package, rawDesired.Discovery) {
	...
	if dcl.IsZeroValue(rawDesired.Package) {
		// check if anything else is set
		if dcl.AnySet(rawDesired.Vulnerability, rawDesired.Build, rawDesired.Discovery) {
	...
	if dcl.IsZeroValue(rawDesired.Discovery) {
		// check if anything else is set
		if dcl.AnySet(rawDesired.Vulnerability, rawDesired.Build, rawDesired.Package) {
