...
func instanceGetURL(userBasePath string, r *Instance) (string, error) {
	...
	return dcl.URL("projects/{{project}}/locations/{{location}}/instances/{{name}}", "https://redis.googleapis.com/v1beta1/", userBasePath, params), nil
}
...
func instanceListURL(userBasePath, project, location string) (string, error) {
	...
	return dcl.URL("projects/{{project}}/locations/{{location}}/instances", "https://redis.googleapis.com/v1beta1/", userBasePath, params), nil
...
func instanceCreateURL(userBasePath, project, location, name string) (string, error) {
	...
	return dcl.URL("projects/{{project}}/locations/{{location}}/instances?instanceId={{name}}", "https://redis.googleapis.com/v1beta1/", userBasePath, params), nil
...
func (op *updateInstanceUpdateInstanceOperation) do(ctx context.Context, r *Instance, c *Client) error {
	...
	err = o.Wait(ctx, c.Config, "https://redis.googleapis.com/v1beta1/", "GET")
...
func (op *deleteInstanceOperation) do(ctx context.Context, r *Instance, c *Client) error {
	...
	if err := o.Wait(ctx, c.Config, "https://redis.googleapis.com/v1beta1/", "GET"); err != nil {
		return err
	}
	_, err = c.GetInstance(ctx, r.urlNormalized())
	if !dcl.IsNotFound(err) {
...
func canonicalizeInstanceDesiredState(rawDesired, rawInitial *Instance, opts ...dcl.ApplyOption) (*Instance, error) {
	...
	if dcl.IsZeroValue(rawDesired.LocationId) {
		rawDesired.LocationId = rawInitial.LocationId
	}
	if dcl.IsZeroValue(rawDesired.AlternativeLocationId) {
		rawDesired.AlternativeLocationId = rawInitial.AlternativeLocationId
	}
	if dcl.IsZeroValue(rawDesired.RedisVersion) {
		rawDesired.RedisVersion = rawInitial.RedisVersion
	}
	if dcl.IsZeroValue(rawDesired.ReservedIPRange) {
	...
	if dcl.IsZeroValue(rawDesired.CurrentLocationId) {
	...
	if dcl.IsZeroValue(rawDesired.AuthorizedNetwork) {
		rawDesired.AuthorizedNetwork = rawInitial.AuthorizedNetwork
	}
	if dcl.IsZeroValue(rawDesired.PersistenceIamIdentity) {
...
func canonicalizeInstanceServerCaCerts(des, initial *InstanceServerCaCerts, opts ...dcl.ApplyOption) *InstanceServerCaCerts {
	...
	if dcl.IsZeroValue(des.SerialNumber) {
...
func compareInstanceServerCaCerts(c *Client, desired, actual *InstanceServerCaCerts) bool {
	...
	if !reflect.DeepEqual(desired.SerialNumber, actual.SerialNumber) && !dcl.IsZeroValue(desired.SerialNumber) && !(dcl.IsEmptyValueIndirect(desired.SerialNumber) && dcl.IsZeroValue(actual.SerialNumber)) {
...
func compareInstanceMaintenancePolicyWeeklyMaintenanceWindowStartTime(c *Client, desired, actual *InstanceMaintenancePolicyWeeklyMaintenanceWindowStartTime) bool {
	...
	if !reflect.DeepEqual(desired.Hours, actual.Hours) && !dcl.IsZeroValue(desired.Hours) && !(dcl.IsEmptyValueIndirect(desired.Hours) && dcl.IsZeroValue(actual.Hours)) {
...
func (r *Instance) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "UpdateInstance" {
		...
		return dcl.URL("projects/{{project}}/locations/{{location}}/instances/{{name}}", "https://redis.googleapis.com/v1beta1/", userBasePath, fields), nil
...
func flattenInstanceConnectModeEnumSlice(c *Client, i interface{}) []InstanceConnectModeEnum {
	...
	for _, item := range a {
		items = append(items, *flattenInstanceConnectModeEnum(item.(map[string]interface{})))
	}
...
func flattenInstanceTransitEncryptionModeEnumSlice(c *Client, i interface{}) []InstanceTransitEncryptionModeEnum {
	...
	for _, item := range a {
		items = append(items, *flattenInstanceTransitEncryptionModeEnum(item.(map[string]interface{})))
	}
...
func flattenInstanceMaintenancePolicyWeeklyMaintenanceWindowDayEnumSlice(c *Client, i interface{}) []InstanceMaintenancePolicyWeeklyMaintenanceWindowDayEnum {
	...
	for _, item := range a {
		items = append(items, *flattenInstanceMaintenancePolicyWeeklyMaintenanceWindowDayEnum(item.(map[string]interface{})))
	}
