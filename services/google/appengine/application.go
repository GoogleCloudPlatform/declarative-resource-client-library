...
func applyApplicationHelper(c *Client, ctx context.Context, rawDesired *Application, opts ...dcl.ApplyOption) (*Application, error) {
	...
	initial, desired, fieldDiffs, err := c.applicationDiffsForRawDesired(ctx, rawDesired, opts...)
	if err != nil {
		return nil, fmt.Errorf("failed to create a diff: %w", err)
	}

	diffs, err := convertFieldDiffsToApplicationDiffs(c.Config, fieldDiffs, opts)
	if err != nil {
		return nil, err
	}
