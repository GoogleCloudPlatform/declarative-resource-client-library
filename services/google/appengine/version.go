...
func applyVersionHelper(c *Client, ctx context.Context, rawDesired *Version, opts ...dcl.ApplyOption) (*Version, error) {
	...
	initial, desired, fieldDiffs, err := c.versionDiffsForRawDesired(ctx, rawDesired, opts...)
	if err != nil {
		return nil, fmt.Errorf("failed to create a diff: %w", err)
	}

	diffs, err := convertFieldDiffsToVersionDiffs(c.Config, fieldDiffs, opts)
	if err != nil {
		return nil, err
	}
