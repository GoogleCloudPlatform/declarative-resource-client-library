...
func (op *deleteAwsNodePoolOperation) do(ctx context.Context, r *AwsNodePool, c *Client) error {
	...
	if err != nil {
		if dcl.IsNotFound(err) {
			c.Config.Logger.InfoWithContextf(ctx, "AwsNodePool not found, returning. Original error: %v", err)
			return nil
		}
		c.Config.Logger.WarningWithContextf(ctx, "GetAwsNodePool checking for existence. error: %v", err)
		return err
	}
...
func (c *Client) awsNodePoolDiffsForRawDesired(ctx context.Context, rawDesired *AwsNodePool, opts ...dcl.ApplyOption) (initial, desired *AwsNodePool, diffs []*dcl.FieldDiff, err error) {
	...
	if sh := dcl.FetchStateHint(opts); sh != nil {
		if r, ok := sh.(*AwsNodePool); !ok {
			c.Config.Logger.WarningWithContextf(ctx, "Initial state hint was of the wrong type; expected AwsNodePool, got %T", sh)
		} else {
			fetchState = r
		}
	}
	...
	if rawInitial == nil {
		if !dcl.IsNotFound(err) {
			c.Config.Logger.WarningWithContextf(ctx, "Failed to retrieve whether a AwsNodePool resource already exists: %s", err)
			return nil, nil, nil, fmt.Errorf("failed to retrieve AwsNodePool resource: %v", err)
		}
		c.Config.Logger.InfoWithContext(ctx, "Found that AwsNodePool resource did not exist.")
		// Perform canonicalization to pick up defaults.
		desired, err = canonicalizeAwsNodePoolDesiredState(rawDesired, rawInitial)
		return nil, desired, nil, err
	}
	c.Config.Logger.InfoWithContextf(ctx, "Found initial state for AwsNodePool: %v", rawInitial)
	c.Config.Logger.InfoWithContextf(ctx, "Initial desired state for AwsNodePool: %v", rawDesired)
	...
	c.Config.Logger.InfoWithContextf(ctx, "Canonicalized initial state for AwsNodePool: %v", initial)
	...
	c.Config.Logger.InfoWithContextf(ctx, "Canonicalized desired state for AwsNodePool: %v", desired)
