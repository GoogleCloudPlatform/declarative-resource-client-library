...
func canonicalizeEndpointDeployedModels(des, initial *EndpointDeployedModels, opts ...dcl.ApplyOption) *EndpointDeployedModels {
	...
	if dcl.IsZeroValue(des.SharedResources) || (dcl.IsEmptyValueIndirect(des.SharedResources) && dcl.IsEmptyValueIndirect(initial.SharedResources)) {
		// Desired and initial values are equivalent, so set canonical desired value to initial value.
		cDes.SharedResources = initial.SharedResources
	} else {
		cDes.SharedResources = des.SharedResources
	}
	if dcl.BoolCanonicalize(des.EnableContainerLogging, initial.EnableContainerLogging) || dcl.IsZeroValue(des.EnableContainerLogging) {
		cDes.EnableContainerLogging = initial.EnableContainerLogging
	} else {
		cDes.EnableContainerLogging = des.EnableContainerLogging
	}
...
func canonicalizeNewEndpointDeployedModels(c *Client, des, nw *EndpointDeployedModels) *EndpointDeployedModels {
	...
	if dcl.BoolCanonicalize(des.EnableContainerLogging, nw.EnableContainerLogging) {
		nw.EnableContainerLogging = des.EnableContainerLogging
	}
...
func compareEndpointDeployedModelsNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.SharedResources, actual.SharedResources, dcl.DiffInfo{Type: "ReferenceType", OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("SharedResources")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.EnableContainerLogging, actual.EnableContainerLogging, dcl.DiffInfo{OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("EnableContainerLogging")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func expandEndpointDeployedModels(c *Client, f *EndpointDeployedModels, res *Endpoint) (map[string]interface{}, error) {
	...
	if v := f.SharedResources; !dcl.IsEmptyValueIndirect(v) {
		m["sharedResources"] = v
	}
	if v := f.EnableContainerLogging; !dcl.IsEmptyValueIndirect(v) {
		m["enableContainerLogging"] = v
	}
...
func flattenEndpointDeployedModels(c *Client, i interface{}, res *Endpoint) *EndpointDeployedModels {
	...
	r.SharedResources = dcl.FlattenString(m["sharedResources"])
	r.EnableContainerLogging = dcl.FlattenBool(m["enableContainerLogging"])
