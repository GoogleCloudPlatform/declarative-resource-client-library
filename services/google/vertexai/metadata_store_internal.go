...
func diffMetadataStore(c *Client, desired, actual *MetadataStore, opts ...dcl.ApplyOption) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.EncryptionSpec, actual.EncryptionSpec, dcl.Info{ObjectFunction: compareMetadataStoreEncryptionSpecNewStyle, EmptyObject: EmptyMetadataStoreEncryptionSpec, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("EncryptionSpec")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
	...
	if ds, err := dcl.Diff(desired.State, actual.State, dcl.Info{OutputOnly: true, ObjectFunction: compareMetadataStoreStateNewStyle, EmptyObject: EmptyMetadataStoreState, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("State")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
...
func compareMetadataStoreStateNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.DiskUtilizationBytes, actual.DiskUtilizationBytes, dcl.Info{OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("DiskUtilizationBytes")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
