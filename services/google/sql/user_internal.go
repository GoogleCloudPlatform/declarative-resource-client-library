...
func userGetURL(userBasePath string, r *User) string {
	...
	return dcl.URL("projects/{{project}}/instances/{{instance}}/users?fetchId=%s%s", "https://www.googleapis.com/sql/v1beta4/", userBasePath, params)
}

func userListURL(userBasePath, project, instance string) string {
	params := map[string]interface{}{
		"project":  project,
		"instance": instance,
	}
	return dcl.URL("projects/{{project}}/instances/{{instance}}/users", "https://www.googleapis.com/sql/v1beta4/", userBasePath, params)
}

func userCreateURL(userBasePath, project, instance string) string {
	params := map[string]interface{}{
		"project":  project,
		"instance": instance,
	}
...
func userDeleteURL(userBasePath string, r *User) string {
	params := map[string]interface{}{
		"project":  dcl.ValueOrEmptyString(r.Project),
		"host":     dcl.ValueOrEmptyString(r.Host),
		"instance": dcl.ValueOrEmptyString(r.Instance),
	...
	return dcl.URL("projects/{{project}}/instances/{{instance}}/users?host={{host}}&name={{name}}", "https://www.googleapis.com/sql/v1beta4/", userBasePath, params)
}
...
func (c *Client) listUserRaw(ctx context.Context, project, instance, pageToken string, pageSize int32) ([]byte, error) {
	u := userListURL(c.Config.BasePath, project, instance)
...
func (op *deleteUserOperation) do(ctx context.Context, r *User, c *Client) error {
	...
	u := userDeleteURL(c.Config.BasePath, r.urlNormalized())
...
func (op *createUserOperation) do(ctx context.Context, r *User, c *Client) error {
	...
	u := userCreateURL(c.Config.BasePath, project, instance)
...
func (c *Client) getUserRaw(ctx context.Context, r *User) ([]byte, error) {
	u := userGetURL(c.Config.BasePath, r.urlNormalized())
	resp, err := dcl.SendRequest(ctx, c.Config, "GET", u, &bytes.Buffer{}, c.Config.Retry)
	if err != nil {
		return nil, err
	}
...
func canonicalizeUserDesiredState(rawDesired, rawInitial *User) (*User, error) {

	if rawInitial == nil {
		// Since the initial state is empty, the desired state is all we have.
		// We canonicalize the remaining nested objects with nil to pick up defaults.
		rawDesired.SqlserverUserDetails = canonicalizeUserSqlserverUserDetails(rawDesired.SqlserverUserDetails, nil)
	...
	rawDesired.SqlserverUserDetails = canonicalizeUserSqlserverUserDetails(rawDesired.SqlserverUserDetails, rawInitial.SqlserverUserDetails)
...
func canonicalizeUserNewState(rawNew, rawDesired *User) (*User, error) {
	...
	if dcl.IsEmptyValueIndirect(rawNew.SqlserverUserDetails) && dcl.IsEmptyValueIndirect(rawDesired.SqlserverUserDetails) {
		...
		rawNew.SqlserverUserDetails = canonicalizeNewUserSqlserverUserDetails(rawDesired.SqlserverUserDetails, rawNew.SqlserverUserDetails)
	}
...
func canonicalizeUserSqlserverUserDetails(des, initial *UserSqlserverUserDetails) *UserSqlserverUserDetails {
	if des == nil {
		return initial
	}
	if des.empty {
...
func canonicalizeNewUserSqlserverUserDetails(des, nw *UserSqlserverUserDetails) *UserSqlserverUserDetails {
	if des == nil || nw == nil {
		return nw
	}
...
func compareUserSqlserverUserDetailsSlice(c *Client, desired, actual []UserSqlserverUserDetails) bool {
	...
	for i := 0; i < len(desired); i++ {
		if compareUserSqlserverUserDetails(c, &desired[i], &desired[i]) {
