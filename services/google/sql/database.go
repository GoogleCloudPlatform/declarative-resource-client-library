...
func (c *Client) GetDatabase(ctx context.Context, r *Database) (*Database, error) {
	...
	result, err = canonicalizeDatabaseNewState(result, r)
	if err != nil {
		return nil, err
	}
...
func (c *Client) ApplyDatabase(ctx context.Context, rawDesired *Database, opts ...dcl.ApplyOption) (*Database, error) {
	...
	initial, desired, diffs, err := c.databaseDiffsForRawDesired(ctx, rawDesired, opts...)
	if err != nil {
	...
	newState, err := canonicalizeDatabaseNewState(rawNew, rawDesired)
	if err != nil {
		return nil, err
	}
