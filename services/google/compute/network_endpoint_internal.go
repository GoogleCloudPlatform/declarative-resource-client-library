...
func networkEndpointGetURL(userBasePath string, r *NetworkEndpoint) (string, error) {
	...
	if dcl.IsRegion(r.Location) {
		return dcl.URL("projects/{{project}}/regions/{{location}}/networkEndpointGroups/{{group}}/listNetworkEndpoints", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
	}

	if dcl.IsZone(r.Location) {
		return dcl.URL("projects/{{project}}/zones/{{location}}/networkEndpointGroups/{{group}}/listNetworkEndpoints", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
	}
...
func networkEndpointCreateURL(userBasePath, project, location, group string) (string, error) {
	...
	if dcl.IsRegion(&location) {
		return dcl.URL("projects/{{project}}/regions/{{location}}/networkEndpointGroups/{{group}}/attachNetworkEndpoints", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
	}

	if dcl.IsZone(&location) {
		return dcl.URL("projects/{{project}}/zones/{{location}}/networkEndpointGroups/{{group}}/attachNetworkEndpoints", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
	}
...
func networkEndpointDeleteURL(userBasePath string, r *NetworkEndpoint) (string, error) {
	...
	if dcl.IsRegion(r.Location) {
		return dcl.URL("projects/{{project}}/regions/{{location}}/networkEndpointGroups/{{group}}/detachNetworkEndpoints", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
	}

	if dcl.IsZone(r.Location) {
		return dcl.URL("projects/{{project}}/zones/{{location}}/networkEndpointGroups/{{group}}/detachNetworkEndpoints", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
	}
...
func diffNetworkEndpoint(c *Client, desired, actual *NetworkEndpoint, opts ...dcl.ApplyOption) ([]networkEndpointDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Port, actual.Port, dcl.Info{OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("Port")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToNetworkEndpointDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func convertFieldDiffToNetworkEndpointDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]networkEndpointDiff, error) {
	var diffs []networkEndpointDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := networkEndpointDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTonetworkEndpointApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTonetworkEndpointApiOperation(op string, opts ...dcl.ApplyOption) (networkEndpointApiOperation, error) {
	switch op {
