...
func convertFieldDiffToVpnGatewayOp(ops []string, fds []*dcl.FieldDiff, opts []dcl.ApplyOption) ([]vpnGatewayDiff, error) {
	...
	for _, op := range ops {
		...
		if op == "Recreate" {
			...
			op, err := convertOpNameTovpnGatewayApiOperation(op, fds, opts...)
			if err != nil {
				return diffs, err
			}
			diff.UpdateOp = op
...
func convertOpNameTovpnGatewayApiOperation(op string, diffs []*dcl.FieldDiff, opts ...dcl.ApplyOption) (vpnGatewayApiOperation, error) {
	switch op {

	case "updateVpnGatewaySetLabelsOperation":
		return &updateVpnGatewaySetLabelsOperation{Diffs: diffs}, nil
