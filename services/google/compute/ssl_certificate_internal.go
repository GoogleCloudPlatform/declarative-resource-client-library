...
func diffSslCertificate(c *Client, desired, actual *SslCertificate, opts ...dcl.ApplyOption) ([]sslCertificateDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Id, actual.Id, dcl.Info{OutputOnly: true, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("Id")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToSslCertificateDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
	...
	if ds, err := dcl.Diff(desired.SelfManaged, actual.SelfManaged, dcl.Info{ObjectFunction: compareSslCertificateSelfManagedNewStyle, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("SelfManaged")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
...
func convertFieldDiffToSslCertificateDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]sslCertificateDiff, error) {
	var diffs []sslCertificateDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := sslCertificateDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTosslCertificateApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTosslCertificateApiOperation(op string, opts ...dcl.ApplyOption) (sslCertificateApiOperation, error) {
	switch op {
