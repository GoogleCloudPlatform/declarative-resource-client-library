...
func routerPeerGetURL(userBasePath string, r *RouterPeer) (string, error) {
	...
	return dcl.URL("projects/{{project}}/regions/{{region}}/routers/{{router}}", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
}
...
func (c *Client) routerPeerDiffsForRawDesired(ctx context.Context, rawDesired *RouterPeer, opts ...dcl.ApplyOption) (initial, desired *RouterPeer, diffs []routerPeerDiff, err error) {
...
func diffRouterPeer(c *Client, desired, actual *RouterPeer, opts ...dcl.ApplyOption) ([]routerPeerDiff, error) {
	...
	if ds, err := dcl.Diff(desired.CreationTimestamp, actual.CreationTimestamp, dcl.Info{OutputOnly: true, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("CreationTimestamp")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToRouterPeerDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func (r *RouterPeer) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "update" {
		...
		return dcl.URL("projects/{{project}}/regions/{{region}}/routers/{{router}}", "https://www.googleapis.com/compute/v1/", userBasePath, fields), nil
...
func convertFieldDiffToRouterPeerDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]routerPeerDiff, error) {
	var diffs []routerPeerDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := routerPeerDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTorouterPeerApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTorouterPeerApiOperation(op string, opts ...dcl.ApplyOption) (routerPeerApiOperation, error) {
	switch op {

	case "updateRouterPeerUpdateOperation":
		return &updateRouterPeerUpdateOperation{}, nil
