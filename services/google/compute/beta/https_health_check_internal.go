...
func convertFieldDiffToHttpsHealthCheckOp(ops []string, fds []*dcl.FieldDiff, opts []dcl.ApplyOption) ([]httpsHealthCheckDiff, error) {
	...
	for _, op := range ops {
		...
		if op == "Recreate" {
			...
			op, err := convertOpNameTohttpsHealthCheckApiOperation(op, fds, opts...)
			if err != nil {
				return diffs, err
			}
			diff.UpdateOp = op
...
func convertOpNameTohttpsHealthCheckApiOperation(op string, diffs []*dcl.FieldDiff, opts ...dcl.ApplyOption) (httpsHealthCheckApiOperation, error) {
	switch op {

	case "updateHttpsHealthCheckUpdateOperation":
		return &updateHttpsHealthCheckUpdateOperation{Diffs: diffs}, nil
