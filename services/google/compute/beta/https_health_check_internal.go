...
func diffHttpsHealthCheck(c *Client, desired, actual *HttpsHealthCheck, opts ...dcl.ApplyOption) ([]httpsHealthCheckDiff, error) {
	...
	if ds, err := dcl.Diff(desired.CheckIntervalSec, actual.CheckIntervalSec, dcl.Info{OperationSelector: dcl.TriggersOperation("updateHttpsHealthCheckUpdateOperation")}, fn.AddNest("CheckIntervalSec")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToHttpsHealthCheckDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func convertFieldDiffToHttpsHealthCheckDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]httpsHealthCheckDiff, error) {
	var diffs []httpsHealthCheckDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := httpsHealthCheckDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTohttpsHealthCheckApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTohttpsHealthCheckApiOperation(op string, opts ...dcl.ApplyOption) (httpsHealthCheckApiOperation, error) {
	switch op {

	case "updateHttpsHealthCheckUpdateOperation":
		return &updateHttpsHealthCheckUpdateOperation{}, nil
