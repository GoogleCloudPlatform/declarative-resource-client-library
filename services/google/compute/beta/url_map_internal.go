...
func urlMapGetURL(userBasePath string, r *UrlMap) (string, error) {
	...
	return dcl.URL("projects/{{project}}/global/urlMaps/{{name}}", "https://www.googleapis.com/compute/beta/", userBasePath, params), nil
}
...
func urlMapListURL(userBasePath, project string) (string, error) {
	...
	return dcl.URL("projects/{{project}}/global/urlMaps", "https://www.googleapis.com/compute/beta/", userBasePath, params), nil
...
func diffUrlMap(c *Client, desired, actual *UrlMap, opts ...dcl.ApplyOption) ([]urlMapDiff, error) {
	...
	if ds, err := dcl.Diff(desired.DefaultRouteAction, actual.DefaultRouteAction, dcl.Info{ObjectFunction: compareUrlMapDefaultRouteActionNewStyle, OperationSelector: dcl.TriggersOperation("updateUrlMapUpdateOperation")}, fn.AddNest("DefaultRouteAction")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToUrlMapDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func (r *UrlMap) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "Update" {
		...
		return dcl.URL("projects/{{project}}/global/urlMaps/{{name}}", "https://www.googleapis.com/compute/beta/", userBasePath, fields), nil
...
func convertFieldDiffToUrlMapDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]urlMapDiff, error) {
	var diffs []urlMapDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := urlMapDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTourlMapApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTourlMapApiOperation(op string, opts ...dcl.ApplyOption) (urlMapApiOperation, error) {
	switch op {

	case "updateUrlMapUpdateOperation":
		return &updateUrlMapUpdateOperation{}, nil
