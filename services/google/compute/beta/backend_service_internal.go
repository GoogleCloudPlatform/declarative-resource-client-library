...
func (r *BackendService) validate() error {
	...
	if !dcl.IsEmptyValueIndirect(r.Subsetting) {
		if err := r.Subsetting.validate(); err != nil {
			return err
		}
	}
	if !dcl.IsEmptyValueIndirect(r.ConnectionTrackingPolicy) {
		if err := r.ConnectionTrackingPolicy.validate(); err != nil {
			return err
		}
	}
...
func (r *BackendServiceCircuitBreakers) validate() error {
	if !dcl.IsEmptyValueIndirect(r.ConnectTimeout) {
		if err := r.ConnectTimeout.validate(); err != nil {
			return err
		}
	}
	return nil
}
func (r *BackendServiceCircuitBreakersConnectTimeout) validate() error {
	return nil
}
...
func (r *BackendServiceSubsetting) validate() error {
	return nil
}
func (r *BackendServiceConnectionTrackingPolicy) validate() error {
	return nil
}
...
func backendServiceGetURL(userBasePath string, r *BackendService) (string, error) {
	...
	if dcl.IsRegion(r.Location) {
		return dcl.URL("projects/{{project}}/regions/{{location}}/backendServices/{{name}}", "https://www.googleapis.com/compute/beta/", userBasePath, params), nil
	}

	return dcl.URL("projects/{{project}}/global/backendServices/{{name}}", "https://www.googleapis.com/compute/beta/", userBasePath, params), nil
}
...
func backendServiceListURL(userBasePath, project, location string) (string, error) {
	...
	if dcl.IsRegion(&location) {
		return dcl.URL("projects/{{project}}/regions/{{location}}/backendServices", "https://www.googleapis.com/compute/beta/", userBasePath, params), nil
	}

	return dcl.URL("projects/{{project}}/global/backendServices", "https://www.googleapis.com/compute/beta/", userBasePath, params), nil
...
func newUpdateBackendServiceUpdateRequest(ctx context.Context, f *BackendService, c *Client) (map[string]interface{}, error) {
	...
	if v, err := expandBackendServiceSubsetting(c, f.Subsetting); err != nil {
		return nil, fmt.Errorf("error expanding Subsetting into subsetting: %w", err)
	} else if !dcl.IsEmptyValueIndirect(v) {
		req["subsetting"] = v
	}
	if v, err := expandBackendServiceConnectionTrackingPolicy(c, f.ConnectionTrackingPolicy); err != nil {
		return nil, fmt.Errorf("error expanding ConnectionTrackingPolicy into connectionTrackingPolicy: %w", err)
	} else if !dcl.IsEmptyValueIndirect(v) {
		req["connectionTrackingPolicy"] = v
	}
...
func canonicalizeBackendServiceDesiredState(rawDesired, rawInitial *BackendService, opts ...dcl.ApplyOption) (*BackendService, error) {
	...
	if rawInitial == nil {
		...
		rawDesired.Subsetting = canonicalizeBackendServiceSubsetting(rawDesired.Subsetting, nil, opts...)
		rawDesired.ConnectionTrackingPolicy = canonicalizeBackendServiceConnectionTrackingPolicy(rawDesired.ConnectionTrackingPolicy, nil, opts...)
	...
	canonicalDesired := &BackendService{}
	...
	if dcl.IsZeroValue(rawDesired.Backends) {
		canonicalDesired.Backends = rawInitial.Backends
	} else {
		canonicalDesired.Backends = rawDesired.Backends
	}
	...
	if dcl.IsZeroValue(rawDesired.Protocol) {
		canonicalDesired.Protocol = rawInitial.Protocol
	} else {
		canonicalDesired.Protocol = rawDesired.Protocol
	}
	if dcl.StringCanonicalize(rawDesired.PortName, rawInitial.PortName) {
		canonicalDesired.PortName = rawInitial.PortName
	} else {
		canonicalDesired.PortName = rawDesired.PortName
	}
	...
	if dcl.IsZeroValue(rawDesired.AffinityCookieTtlSec) {
		canonicalDesired.AffinityCookieTtlSec = rawInitial.AffinityCookieTtlSec
	} else {
		canonicalDesired.AffinityCookieTtlSec = rawDesired.AffinityCookieTtlSec
	}
	...
	canonicalDesired.FailoverPolicy = canonicalizeBackendServiceFailoverPolicy(rawDesired.FailoverPolicy, rawInitial.FailoverPolicy, opts...)
	...
	canonicalDesired.ConnectionDraining = canonicalizeBackendServiceConnectionDraining(rawDesired.ConnectionDraining, rawInitial.ConnectionDraining, opts...)
	canonicalDesired.Iap = canonicalizeBackendServiceIap(rawDesired.Iap, rawInitial.Iap, opts...)
	canonicalDesired.CdnPolicy = canonicalizeBackendServiceCdnPolicy(rawDesired.CdnPolicy, rawInitial.CdnPolicy, opts...)
	if dcl.IsZeroValue(rawDesired.CustomRequestHeaders) {
		canonicalDesired.CustomRequestHeaders = rawInitial.CustomRequestHeaders
	} else {
		canonicalDesired.CustomRequestHeaders = rawDesired.CustomRequestHeaders
	}
	if dcl.IsZeroValue(rawDesired.CustomResponseHeaders) {
		canonicalDesired.CustomResponseHeaders = rawInitial.CustomResponseHeaders
	} else {
		canonicalDesired.CustomResponseHeaders = rawDesired.CustomResponseHeaders
	}
	if dcl.NameToSelfLink(rawDesired.SecurityPolicy, rawInitial.SecurityPolicy) {
		canonicalDesired.SecurityPolicy = rawInitial.SecurityPolicy
	} else {
		canonicalDesired.SecurityPolicy = rawDesired.SecurityPolicy
	}
	canonicalDesired.LogConfig = canonicalizeBackendServiceLogConfig(rawDesired.LogConfig, rawInitial.LogConfig, opts...)
	canonicalDesired.SecuritySettings = canonicalizeBackendServiceSecuritySettings(rawDesired.SecuritySettings, rawInitial.SecuritySettings, opts...)
	if dcl.IsZeroValue(rawDesired.LocalityLbPolicy) {
		canonicalDesired.LocalityLbPolicy = rawInitial.LocalityLbPolicy
	} else {
		canonicalDesired.LocalityLbPolicy = rawDesired.LocalityLbPolicy
	}
	canonicalDesired.ConsistentHash = canonicalizeBackendServiceConsistentHash(rawDesired.ConsistentHash, rawInitial.ConsistentHash, opts...)
	canonicalDesired.CircuitBreakers = canonicalizeBackendServiceCircuitBreakers(rawDesired.CircuitBreakers, rawInitial.CircuitBreakers, opts...)
	canonicalDesired.OutlierDetection = canonicalizeBackendServiceOutlierDetection(rawDesired.OutlierDetection, rawInitial.OutlierDetection, opts...)
	...
	canonicalDesired.Subsetting = canonicalizeBackendServiceSubsetting(rawDesired.Subsetting, rawInitial.Subsetting, opts...)
	canonicalDesired.ConnectionTrackingPolicy = canonicalizeBackendServiceConnectionTrackingPolicy(rawDesired.ConnectionTrackingPolicy, rawInitial.ConnectionTrackingPolicy, opts...)
	canonicalDesired.MaxStreamDuration = canonicalizeBackendServiceMaxStreamDuration(rawDesired.MaxStreamDuration, rawInitial.MaxStreamDuration, opts...)
...
func canonicalizeBackendServiceNewState(c *Client, rawNew, rawDesired *BackendService) (*BackendService, error) {
	...
	if dcl.IsEmptyValueIndirect(rawNew.Subsetting) && dcl.IsEmptyValueIndirect(rawDesired.Subsetting) {
		rawNew.Subsetting = rawDesired.Subsetting
	} else {
		rawNew.Subsetting = canonicalizeNewBackendServiceSubsetting(c, rawDesired.Subsetting, rawNew.Subsetting)
	}

	if dcl.IsEmptyValueIndirect(rawNew.ConnectionTrackingPolicy) && dcl.IsEmptyValueIndirect(rawDesired.ConnectionTrackingPolicy) {
		rawNew.ConnectionTrackingPolicy = rawDesired.ConnectionTrackingPolicy
	} else {
		rawNew.ConnectionTrackingPolicy = canonicalizeNewBackendServiceConnectionTrackingPolicy(c, rawDesired.ConnectionTrackingPolicy, rawNew.ConnectionTrackingPolicy)
	}
...
func canonicalizeBackendServiceFailoverPolicy(des, initial *BackendServiceFailoverPolicy, opts ...dcl.ApplyOption) *BackendServiceFailoverPolicy {
	...
	cDes := &BackendServiceFailoverPolicy{}
	...
	if dcl.BoolCanonicalize(des.DisableConnectionDrainOnFailover, initial.DisableConnectionDrainOnFailover) || dcl.IsZeroValue(des.DisableConnectionDrainOnFailover) {
		cDes.DisableConnectionDrainOnFailover = initial.DisableConnectionDrainOnFailover
	} else {
		cDes.DisableConnectionDrainOnFailover = des.DisableConnectionDrainOnFailover
	}
	if dcl.BoolCanonicalize(des.DropTrafficIfUnhealthy, initial.DropTrafficIfUnhealthy) || dcl.IsZeroValue(des.DropTrafficIfUnhealthy) {
		cDes.DropTrafficIfUnhealthy = initial.DropTrafficIfUnhealthy
	} else {
		cDes.DropTrafficIfUnhealthy = des.DropTrafficIfUnhealthy
	}
	if dcl.IsZeroValue(des.FailoverRatio) {
		des.FailoverRatio = initial.FailoverRatio
	} else {
		cDes.FailoverRatio = des.FailoverRatio
	}
...
func canonicalizeBackendServiceCdnPolicy(des, initial *BackendServiceCdnPolicy, opts ...dcl.ApplyOption) *BackendServiceCdnPolicy {
	...
	cDes := &BackendServiceCdnPolicy{}

	cDes.CacheKeyPolicy = canonicalizeBackendServiceCdnPolicyCacheKeyPolicy(des.CacheKeyPolicy, initial.CacheKeyPolicy, opts...)
...
func canonicalizeBackendServiceCdnPolicyNegativeCachingPolicy(des, initial *BackendServiceCdnPolicyNegativeCachingPolicy, opts ...dcl.ApplyOption) *BackendServiceCdnPolicyNegativeCachingPolicy {
	...
	cDes := &BackendServiceCdnPolicyNegativeCachingPolicy{}
...
func canonicalizeBackendServiceCdnPolicyBypassCacheOnRequestHeaders(des, initial *BackendServiceCdnPolicyBypassCacheOnRequestHeaders, opts ...dcl.ApplyOption) *BackendServiceCdnPolicyBypassCacheOnRequestHeaders {
	...
	cDes := &BackendServiceCdnPolicyBypassCacheOnRequestHeaders{}
...
func canonicalizeBackendServiceLogConfig(des, initial *BackendServiceLogConfig, opts ...dcl.ApplyOption) *BackendServiceLogConfig {
	...
	if dcl.IsZeroValue(des.SampleRate) {
		...
		cDes.SampleRate = des.SampleRate
	}
...
func canonicalizeBackendServiceSecuritySettings(des, initial *BackendServiceSecuritySettings, opts ...dcl.ApplyOption) *BackendServiceSecuritySettings {
	...
	cDes := &BackendServiceSecuritySettings{}

	if dcl.NameToSelfLink(des.ClientTlsPolicy, initial.ClientTlsPolicy) || dcl.IsZeroValue(des.ClientTlsPolicy) {
		cDes.ClientTlsPolicy = initial.ClientTlsPolicy
	} else {
		cDes.ClientTlsPolicy = des.ClientTlsPolicy
	}
	if dcl.StringCanonicalize(des.Authentication, initial.Authentication) || dcl.IsZeroValue(des.Authentication) {
		cDes.Authentication = initial.Authentication
	} else {
		cDes.Authentication = des.Authentication
	}
	if dcl.IsZeroValue(des.SubjectAltNames) {
		des.SubjectAltNames = initial.SubjectAltNames
	} else {
		cDes.SubjectAltNames = des.SubjectAltNames
	}
...
func canonicalizeBackendServiceConsistentHash(des, initial *BackendServiceConsistentHash, opts ...dcl.ApplyOption) *BackendServiceConsistentHash {
	...
	cDes := &BackendServiceConsistentHash{}

	cDes.HttpCookie = canonicalizeBackendServiceConsistentHashHttpCookie(des.HttpCookie, initial.HttpCookie, opts...)
	if dcl.StringCanonicalize(des.HttpHeaderName, initial.HttpHeaderName) || dcl.IsZeroValue(des.HttpHeaderName) {
		cDes.HttpHeaderName = initial.HttpHeaderName
	} else {
		cDes.HttpHeaderName = des.HttpHeaderName
	}
	if dcl.IsZeroValue(des.MinimumRingSize) {
		des.MinimumRingSize = initial.MinimumRingSize
	} else {
		cDes.MinimumRingSize = des.MinimumRingSize
	}
...
func canonicalizeBackendServiceConsistentHashHttpCookie(des, initial *BackendServiceConsistentHashHttpCookie, opts ...dcl.ApplyOption) *BackendServiceConsistentHashHttpCookie {
	...
	cDes := &BackendServiceConsistentHashHttpCookie{}
	...
	cDes.Ttl = canonicalizeBackendServiceConsistentHashHttpCookieTtl(des.Ttl, initial.Ttl, opts...)
...
func canonicalizeBackendServiceConsistentHashHttpCookieTtl(des, initial *BackendServiceConsistentHashHttpCookieTtl, opts ...dcl.ApplyOption) *BackendServiceConsistentHashHttpCookieTtl {
	...
	cDes := &BackendServiceConsistentHashHttpCookieTtl{}
...
func canonicalizeBackendServiceCircuitBreakers(des, initial *BackendServiceCircuitBreakers, opts ...dcl.ApplyOption) *BackendServiceCircuitBreakers {
	...
	cDes := &BackendServiceCircuitBreakers{}

	cDes.ConnectTimeout = canonicalizeBackendServiceCircuitBreakersConnectTimeout(des.ConnectTimeout, initial.ConnectTimeout, opts...)
	if dcl.IsZeroValue(des.MaxRequestsPerConnection) {
		des.MaxRequestsPerConnection = initial.MaxRequestsPerConnection
	} else {
		cDes.MaxRequestsPerConnection = des.MaxRequestsPerConnection
	}
	if dcl.IsZeroValue(des.MaxConnections) {
		des.MaxConnections = initial.MaxConnections
	} else {
	...
	if dcl.IsZeroValue(des.MaxPendingRequests) {
		...
		cDes.MaxPendingRequests = des.MaxPendingRequests
	}
	if dcl.IsZeroValue(des.MaxRequests) {
		des.MaxRequests = initial.MaxRequests
	} else {
		cDes.MaxRequests = des.MaxRequests
	}
...
func canonicalizeBackendServiceCircuitBreakersConnectTimeout(des, initial *BackendServiceCircuitBreakersConnectTimeout, opts ...dcl.ApplyOption) *BackendServiceCircuitBreakersConnectTimeout {
	if des == nil {
		return initial
	}
	if des.empty {
	...
	cDes := &BackendServiceCircuitBreakersConnectTimeout{}
...
func canonicalizeNewBackendServiceCircuitBreakersConnectTimeoutSet(c *Client, des, nw []BackendServiceCircuitBreakersConnectTimeout) []BackendServiceCircuitBreakersConnectTimeout {
	if des == nil {
		return nw
	}
	var reorderedNew []BackendServiceCircuitBreakersConnectTimeout
	for _, d := range des {
		matchedNew := -1
		for idx, n := range nw {
			if diffs, _ := compareBackendServiceCircuitBreakersConnectTimeoutNewStyle(&d, &n, dcl.FieldName{}); len(diffs) == 0 {
				matchedNew = idx
				break
			}
		}
...
func canonicalizeNewBackendServiceCircuitBreakersConnectTimeoutSlice(c *Client, des, nw []BackendServiceCircuitBreakersConnectTimeout) []BackendServiceCircuitBreakersConnectTimeout {
	if des == nil {
		return nw
	}
	...
	var items []BackendServiceCircuitBreakersConnectTimeout
	for i, d := range des {
		n := nw[i]
		items = append(items, *canonicalizeNewBackendServiceCircuitBreakersConnectTimeout(c, &d, &n))
	}
...
func canonicalizeBackendServiceOutlierDetection(des, initial *BackendServiceOutlierDetection, opts ...dcl.ApplyOption) *BackendServiceOutlierDetection {
	...
	if dcl.IsZeroValue(des.EnforcingConsecutiveGatewayFailure) {
		...
		cDes.EnforcingConsecutiveGatewayFailure = des.EnforcingConsecutiveGatewayFailure
	}
...
func canonicalizeBackendServiceOutlierDetectionInterval(des, initial *BackendServiceOutlierDetectionInterval, opts ...dcl.ApplyOption) *BackendServiceOutlierDetectionInterval {
	...
	cDes := &BackendServiceOutlierDetectionInterval{}
...
func canonicalizeBackendServiceOutlierDetectionBaseEjectionTime(des, initial *BackendServiceOutlierDetectionBaseEjectionTime, opts ...dcl.ApplyOption) *BackendServiceOutlierDetectionBaseEjectionTime {
	...
	cDes := &BackendServiceOutlierDetectionBaseEjectionTime{}
...
func canonicalizeBackendServiceSubsetting(des, initial *BackendServiceSubsetting, opts ...dcl.ApplyOption) *BackendServiceSubsetting {
	if des == nil {
		return initial
	}
	if des.empty {
	...
	cDes := &BackendServiceSubsetting{}

	if dcl.IsZeroValue(des.Policy) {
...
func canonicalizeNewBackendServiceSubsettingSet(c *Client, des, nw []BackendServiceSubsetting) []BackendServiceSubsetting {
	if des == nil {
		return nw
	}
	var reorderedNew []BackendServiceSubsetting
	for _, d := range des {
		matchedNew := -1
		for idx, n := range nw {
			if diffs, _ := compareBackendServiceSubsettingNewStyle(&d, &n, dcl.FieldName{}); len(diffs) == 0 {
				matchedNew = idx
				break
			}
		}
...
func canonicalizeNewBackendServiceSubsettingSlice(c *Client, des, nw []BackendServiceSubsetting) []BackendServiceSubsetting {
	if des == nil {
		return nw
	}
	...
	var items []BackendServiceSubsetting
	for i, d := range des {
		n := nw[i]
		items = append(items, *canonicalizeNewBackendServiceSubsetting(c, &d, &n))
	}

	return items
}

func canonicalizeBackendServiceConnectionTrackingPolicy(des, initial *BackendServiceConnectionTrackingPolicy, opts ...dcl.ApplyOption) *BackendServiceConnectionTrackingPolicy {
	if des == nil {
		return initial
	}
	if des.empty {
	...
	cDes := &BackendServiceConnectionTrackingPolicy{}

	if dcl.IsZeroValue(des.TrackingMode) {
		des.TrackingMode = initial.TrackingMode
	} else {
		cDes.TrackingMode = des.TrackingMode
	}
	if dcl.IsZeroValue(des.ConnectionPersistenceOnUnhealthyBackends) {
		des.ConnectionPersistenceOnUnhealthyBackends = initial.ConnectionPersistenceOnUnhealthyBackends
	} else {
		cDes.ConnectionPersistenceOnUnhealthyBackends = des.ConnectionPersistenceOnUnhealthyBackends
	}
	if dcl.IsZeroValue(des.IdleTimeoutSec) {
		des.IdleTimeoutSec = initial.IdleTimeoutSec
	} else {
		cDes.IdleTimeoutSec = des.IdleTimeoutSec
	}

	return cDes
}

func canonicalizeNewBackendServiceConnectionTrackingPolicy(c *Client, des, nw *BackendServiceConnectionTrackingPolicy) *BackendServiceConnectionTrackingPolicy {
	if des == nil || nw == nil {
		return nw
	}
...
func canonicalizeNewBackendServiceConnectionTrackingPolicySet(c *Client, des, nw []BackendServiceConnectionTrackingPolicy) []BackendServiceConnectionTrackingPolicy {
	if des == nil {
		return nw
	}
	var reorderedNew []BackendServiceConnectionTrackingPolicy
	for _, d := range des {
		matchedNew := -1
		for idx, n := range nw {
			if diffs, _ := compareBackendServiceConnectionTrackingPolicyNewStyle(&d, &n, dcl.FieldName{}); len(diffs) == 0 {
				matchedNew = idx
				break
			}
		}
...
func canonicalizeNewBackendServiceConnectionTrackingPolicySlice(c *Client, des, nw []BackendServiceConnectionTrackingPolicy) []BackendServiceConnectionTrackingPolicy {
	if des == nil {
		return nw
	}
	...
	var items []BackendServiceConnectionTrackingPolicy
	for i, d := range des {
		n := nw[i]
		items = append(items, *canonicalizeNewBackendServiceConnectionTrackingPolicy(c, &d, &n))
	}
...
func canonicalizeBackendServiceMaxStreamDuration(des, initial *BackendServiceMaxStreamDuration, opts ...dcl.ApplyOption) *BackendServiceMaxStreamDuration {
	...
	cDes := &BackendServiceMaxStreamDuration{}
...
func compareBackendServiceSecuritySettingsNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Authentication, actual.Authentication, dcl.Info{OperationSelector: dcl.TriggersOperation("updateBackendServiceUpdateOperation")}, fn.AddNest("Authentication")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareBackendServiceCircuitBreakersConnectTimeoutNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	var diffs []*dcl.FieldDiff

	desired, ok := d.(*BackendServiceCircuitBreakersConnectTimeout)
	if !ok {
		desiredNotPointer, ok := d.(BackendServiceCircuitBreakersConnectTimeout)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a BackendServiceCircuitBreakersConnectTimeout or *BackendServiceCircuitBreakersConnectTimeout", d)
		}
		desired = &desiredNotPointer
	}
	actual, ok := a.(*BackendServiceCircuitBreakersConnectTimeout)
	if !ok {
		actualNotPointer, ok := a.(BackendServiceCircuitBreakersConnectTimeout)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a BackendServiceCircuitBreakersConnectTimeout", a)
		}
		actual = &actualNotPointer
	}
...
func compareBackendServiceSubsettingNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	var diffs []*dcl.FieldDiff

	desired, ok := d.(*BackendServiceSubsetting)
	if !ok {
		desiredNotPointer, ok := d.(BackendServiceSubsetting)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a BackendServiceSubsetting or *BackendServiceSubsetting", d)
		}
		desired = &desiredNotPointer
	}
	actual, ok := a.(*BackendServiceSubsetting)
	if !ok {
		actualNotPointer, ok := a.(BackendServiceSubsetting)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a BackendServiceSubsetting", a)
		}
		actual = &actualNotPointer
	}

	if ds, err := dcl.Diff(desired.Policy, actual.Policy, dcl.Info{Type: "EnumType", OperationSelector: dcl.TriggersOperation("updateBackendServiceUpdateOperation")}, fn.AddNest("Policy")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func compareBackendServiceConnectionTrackingPolicyNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	var diffs []*dcl.FieldDiff

	desired, ok := d.(*BackendServiceConnectionTrackingPolicy)
	if !ok {
		desiredNotPointer, ok := d.(BackendServiceConnectionTrackingPolicy)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a BackendServiceConnectionTrackingPolicy or *BackendServiceConnectionTrackingPolicy", d)
		}
		desired = &desiredNotPointer
	}
	actual, ok := a.(*BackendServiceConnectionTrackingPolicy)
	if !ok {
		actualNotPointer, ok := a.(BackendServiceConnectionTrackingPolicy)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a BackendServiceConnectionTrackingPolicy", a)
		}
		actual = &actualNotPointer
	}

	if ds, err := dcl.Diff(desired.TrackingMode, actual.TrackingMode, dcl.Info{Type: "EnumType", OperationSelector: dcl.TriggersOperation("updateBackendServiceUpdateOperation")}, fn.AddNest("TrackingMode")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.ConnectionPersistenceOnUnhealthyBackends, actual.ConnectionPersistenceOnUnhealthyBackends, dcl.Info{Type: "EnumType", OperationSelector: dcl.TriggersOperation("updateBackendServiceUpdateOperation")}, fn.AddNest("ConnectionPersistenceOnUnhealthyBackends")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
	...
	if ds, err := dcl.Diff(desired.IdleTimeoutSec, actual.IdleTimeoutSec, dcl.Info{OperationSelector: dcl.TriggersOperation("updateBackendServiceUpdateOperation")}, fn.AddNest("IdleTimeoutSec")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func (r *BackendService) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "Update" {
		...
		if dcl.IsRegion(r.Location) {
			return dcl.URL("projects/{{project}}/regions/{{location}}/backendServices/{{name}}", "https://www.googleapis.com/compute/beta/", userBasePath, fields), nil
		}

		return dcl.URL("projects/{{project}}/global/backendServices/{{name}}", "https://www.googleapis.com/compute/beta/", userBasePath, fields), nil
...
func flattenBackendService(c *Client, i interface{}) *BackendService {
	...
	res.Subsetting = flattenBackendServiceSubsetting(c, m["subsetting"])
	res.ConnectionTrackingPolicy = flattenBackendServiceConnectionTrackingPolicy(c, m["connectionTrackingPolicy"])
...
func expandBackendServiceCircuitBreakers(c *Client, f *BackendServiceCircuitBreakers) (map[string]interface{}, error) {
	...
	m := make(map[string]interface{})
	if v, err := expandBackendServiceCircuitBreakersConnectTimeout(c, f.ConnectTimeout); err != nil {
		return nil, fmt.Errorf("error expanding ConnectTimeout into connectTimeout: %w", err)
	} else if !dcl.IsEmptyValueIndirect(v) {
		m["connectTimeout"] = v
	}
...
// expandBackendServiceCircuitBreakersConnectTimeoutMap expands the contents of BackendServiceCircuitBreakersConnectTimeout into a JSON
// request object.
func expandBackendServiceCircuitBreakersConnectTimeoutMap(c *Client, f map[string]BackendServiceCircuitBreakersConnectTimeout) (map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
	...
	items := make(map[string]interface{})
	for k, item := range f {
		i, err := expandBackendServiceCircuitBreakersConnectTimeout(c, &item)
		if err != nil {
			return nil, err
		}
		if i != nil {
...
// expandBackendServiceCircuitBreakersConnectTimeoutSlice expands the contents of BackendServiceCircuitBreakersConnectTimeout into a JSON
// request object.
func expandBackendServiceCircuitBreakersConnectTimeoutSlice(c *Client, f []BackendServiceCircuitBreakersConnectTimeout) ([]map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
...
// flattenBackendServiceCircuitBreakersConnectTimeoutMap flattens the contents of BackendServiceCircuitBreakersConnectTimeout from a JSON
// response object.
func flattenBackendServiceCircuitBreakersConnectTimeoutMap(c *Client, i interface{}) map[string]BackendServiceCircuitBreakersConnectTimeout {
	a, ok := i.(map[string]interface{})
	if !ok {
		return map[string]BackendServiceCircuitBreakersConnectTimeout{}
	}
	...
	items := make(map[string]BackendServiceCircuitBreakersConnectTimeout)
	for k, item := range a {
		items[k] = *flattenBackendServiceCircuitBreakersConnectTimeout(c, item.(map[string]interface{}))
	}

	return items
}

// flattenBackendServiceCircuitBreakersConnectTimeoutSlice flattens the contents of BackendServiceCircuitBreakersConnectTimeout from a JSON
// response object.
func flattenBackendServiceCircuitBreakersConnectTimeoutSlice(c *Client, i interface{}) []BackendServiceCircuitBreakersConnectTimeout {
	a, ok := i.([]interface{})
	if !ok {
	...
	items := make([]BackendServiceCircuitBreakersConnectTimeout, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenBackendServiceCircuitBreakersConnectTimeout(c, item.(map[string]interface{})))
	}

	return items
}

// expandBackendServiceCircuitBreakersConnectTimeout expands an instance of BackendServiceCircuitBreakersConnectTimeout into a JSON
// request object.
func expandBackendServiceCircuitBreakersConnectTimeout(c *Client, f *BackendServiceCircuitBreakersConnectTimeout) (map[string]interface{}, error) {
	if dcl.IsEmptyValueIndirect(f) {
		return nil, nil
	}
...
// flattenBackendServiceCircuitBreakersConnectTimeout flattens an instance of BackendServiceCircuitBreakersConnectTimeout from a JSON
// response object.
func flattenBackendServiceCircuitBreakersConnectTimeout(c *Client, i interface{}) *BackendServiceCircuitBreakersConnectTimeout {
	m, ok := i.(map[string]interface{})
	if !ok {
		return nil
	}
...
// expandBackendServiceSubsettingMap expands the contents of BackendServiceSubsetting into a JSON
// request object.
func expandBackendServiceSubsettingMap(c *Client, f map[string]BackendServiceSubsetting) (map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
	...
	items := make(map[string]interface{})
	for k, item := range f {
		i, err := expandBackendServiceSubsetting(c, &item)
		if err != nil {
			return nil, err
		}
		if i != nil {
...
// expandBackendServiceSubsettingSlice expands the contents of BackendServiceSubsetting into a JSON
// request object.
func expandBackendServiceSubsettingSlice(c *Client, f []BackendServiceSubsetting) ([]map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
...
// flattenBackendServiceSubsettingMap flattens the contents of BackendServiceSubsetting from a JSON
// response object.
func flattenBackendServiceSubsettingMap(c *Client, i interface{}) map[string]BackendServiceSubsetting {
	a, ok := i.(map[string]interface{})
	if !ok {
		return map[string]BackendServiceSubsetting{}
	}
	...
	items := make(map[string]BackendServiceSubsetting)
	for k, item := range a {
		items[k] = *flattenBackendServiceSubsetting(c, item.(map[string]interface{}))
	}

	return items
}

// flattenBackendServiceSubsettingSlice flattens the contents of BackendServiceSubsetting from a JSON
// response object.
func flattenBackendServiceSubsettingSlice(c *Client, i interface{}) []BackendServiceSubsetting {
	a, ok := i.([]interface{})
	if !ok {
	...
	items := make([]BackendServiceSubsetting, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenBackendServiceSubsetting(c, item.(map[string]interface{})))
	}

	return items
}

// expandBackendServiceSubsetting expands an instance of BackendServiceSubsetting into a JSON
// request object.
func expandBackendServiceSubsetting(c *Client, f *BackendServiceSubsetting) (map[string]interface{}, error) {
	if dcl.IsEmptyValueIndirect(f) {
		return nil, nil
	}
...
// flattenBackendServiceSubsetting flattens an instance of BackendServiceSubsetting from a JSON
// response object.
func flattenBackendServiceSubsetting(c *Client, i interface{}) *BackendServiceSubsetting {
	m, ok := i.(map[string]interface{})
	if !ok {
		return nil
	}
...
// expandBackendServiceConnectionTrackingPolicyMap expands the contents of BackendServiceConnectionTrackingPolicy into a JSON
// request object.
func expandBackendServiceConnectionTrackingPolicyMap(c *Client, f map[string]BackendServiceConnectionTrackingPolicy) (map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
	...
	items := make(map[string]interface{})
	for k, item := range f {
		i, err := expandBackendServiceConnectionTrackingPolicy(c, &item)
		if err != nil {
			return nil, err
		}
		if i != nil {
...
// expandBackendServiceConnectionTrackingPolicySlice expands the contents of BackendServiceConnectionTrackingPolicy into a JSON
// request object.
func expandBackendServiceConnectionTrackingPolicySlice(c *Client, f []BackendServiceConnectionTrackingPolicy) ([]map[string]interface{}, error) {
	if f == nil {
		return nil, nil
	}
...
// flattenBackendServiceConnectionTrackingPolicyMap flattens the contents of BackendServiceConnectionTrackingPolicy from a JSON
// response object.
func flattenBackendServiceConnectionTrackingPolicyMap(c *Client, i interface{}) map[string]BackendServiceConnectionTrackingPolicy {
	a, ok := i.(map[string]interface{})
	if !ok {
		return map[string]BackendServiceConnectionTrackingPolicy{}
	}
	...
	items := make(map[string]BackendServiceConnectionTrackingPolicy)
	for k, item := range a {
		items[k] = *flattenBackendServiceConnectionTrackingPolicy(c, item.(map[string]interface{}))
	}

	return items
}

// flattenBackendServiceConnectionTrackingPolicySlice flattens the contents of BackendServiceConnectionTrackingPolicy from a JSON
// response object.
func flattenBackendServiceConnectionTrackingPolicySlice(c *Client, i interface{}) []BackendServiceConnectionTrackingPolicy {
	a, ok := i.([]interface{})
	if !ok {
	...
	items := make([]BackendServiceConnectionTrackingPolicy, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenBackendServiceConnectionTrackingPolicy(c, item.(map[string]interface{})))
	}

	return items
}

// expandBackendServiceConnectionTrackingPolicy expands an instance of BackendServiceConnectionTrackingPolicy into a JSON
// request object.
func expandBackendServiceConnectionTrackingPolicy(c *Client, f *BackendServiceConnectionTrackingPolicy) (map[string]interface{}, error) {
	if dcl.IsEmptyValueIndirect(f) {
		return nil, nil
	}

	m := make(map[string]interface{})
	if v := f.TrackingMode; !dcl.IsEmptyValueIndirect(v) {
		m["trackingMode"] = v
	}
	if v := f.ConnectionPersistenceOnUnhealthyBackends; !dcl.IsEmptyValueIndirect(v) {
		m["connectionPersistenceOnUnhealthyBackends"] = v
	}
	if v := f.IdleTimeoutSec; !dcl.IsEmptyValueIndirect(v) {
		m["idleTimeoutSec"] = v
	}
...
// flattenBackendServiceConnectionTrackingPolicy flattens an instance of BackendServiceConnectionTrackingPolicy from a JSON
// response object.
func flattenBackendServiceConnectionTrackingPolicy(c *Client, i interface{}) *BackendServiceConnectionTrackingPolicy {
	m, ok := i.(map[string]interface{})
	if !ok {
		return nil
	}
...
func flattenBackendServiceSubsettingPolicyEnumSlice(c *Client, i interface{}) []BackendServiceSubsettingPolicyEnum {
	a, ok := i.([]interface{})
	if !ok {
		return []BackendServiceSubsettingPolicyEnum{}
	}
	...
	items := make([]BackendServiceSubsettingPolicyEnum, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenBackendServiceSubsettingPolicyEnum(item.(interface{})))
	}

	return items
}

// flattenBackendServiceSubsettingPolicyEnum asserts that an interface is a string, and returns a
// pointer to a *BackendServiceSubsettingPolicyEnum with the same value as that string.
func flattenBackendServiceSubsettingPolicyEnum(i interface{}) *BackendServiceSubsettingPolicyEnum {
	s, ok := i.(string)
	if !ok {
		return BackendServiceSubsettingPolicyEnumRef("")
	}
...
func flattenBackendServiceConnectionTrackingPolicyTrackingModeEnumSlice(c *Client, i interface{}) []BackendServiceConnectionTrackingPolicyTrackingModeEnum {
	a, ok := i.([]interface{})
	if !ok {
		return []BackendServiceConnectionTrackingPolicyTrackingModeEnum{}
	}
	...
	items := make([]BackendServiceConnectionTrackingPolicyTrackingModeEnum, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenBackendServiceConnectionTrackingPolicyTrackingModeEnum(item.(interface{})))
	}

	return items
}

// flattenBackendServiceConnectionTrackingPolicyTrackingModeEnum asserts that an interface is a string, and returns a
// pointer to a *BackendServiceConnectionTrackingPolicyTrackingModeEnum with the same value as that string.
func flattenBackendServiceConnectionTrackingPolicyTrackingModeEnum(i interface{}) *BackendServiceConnectionTrackingPolicyTrackingModeEnum {
	s, ok := i.(string)
	if !ok {
		return BackendServiceConnectionTrackingPolicyTrackingModeEnumRef("")
	}
...
// response object.
func flattenBackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnumSlice(c *Client, i interface{}) []BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum {
	a, ok := i.([]interface{})
	if !ok {
		return []BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum{}
	}
	...
	items := make([]BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenBackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum(item.(interface{})))
	}

	return items
}

// flattenBackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum asserts that an interface is a string, and returns a
// pointer to a *BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum with the same value as that string.
func flattenBackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum(i interface{}) *BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnum {
	s, ok := i.(string)
	if !ok {
		return BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnumRef("")
	}

	return BackendServiceConnectionTrackingPolicyConnectionPersistenceOnUnhealthyBackendsEnumRef(s)
}
...
func convertFieldDiffsToBackendServiceDiffs(config *dcl.Config, fds []*dcl.FieldDiff, opts []dcl.ApplyOption) ([]backendServiceDiff, error) {
	...
	// For each operation name, create a backendServiceDiff which contains the operation.
	for opName, fieldDiffs := range opNamesToFieldDiffs {
		diff := backendServiceDiff{}
		if opName == "Recreate" {
			diff.RequiresRecreate = true
		} else {
			apiOp, err := convertOpNameToBackendServiceApiOperation(opName, fieldDiffs, opts...)
			if err != nil {
				return diffs, err
			}
			diff.UpdateOp = apiOp
...
func convertOpNameToBackendServiceApiOperation(opName string, fieldDiffs []*dcl.FieldDiff, opts ...dcl.ApplyOption) (backendServiceApiOperation, error) {
	switch opName {

	case "updateBackendServiceUpdateOperation":
		return &updateBackendServiceUpdateOperation{FieldDiffs: fieldDiffs}, nil
