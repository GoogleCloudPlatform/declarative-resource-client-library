...
func routeGetURL(userBasePath string, r *Route) (string, error) {
	...
	return dcl.URL("projects/{{project}}/global/routes/{{name}}", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
}
...
func routeListURL(userBasePath, project string) (string, error) {
	...
	return dcl.URL("projects/{{project}}/global/routes", "https://www.googleapis.com/compute/v1/", userBasePath, params), nil
...
func diffRoute(c *Client, desired, actual *Route, opts ...dcl.ApplyOption) ([]routeDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Id, actual.Id, dcl.Info{OutputOnly: true, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("Id")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToRouteDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func convertFieldDiffToRouteDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]routeDiff, error) {
	var diffs []routeDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := routeDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTorouteApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTorouteApiOperation(op string, opts ...dcl.ApplyOption) (routeApiOperation, error) {
	switch op {
