...
// Package cloudbuild contains handwritten support code for the cloud build service.
package cloudbuild
...
func (op *Operation) operate(ctx context.Context) (*dcl.RetryDetails, error) {
	u := dcl.URL(fmt.Sprintf("operations/%s", op.Name), "https://cloudbuild.googleapis.com/v1/", op.config.BasePath, nil)
