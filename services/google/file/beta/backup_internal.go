...
func (op *updateBackupUpdateBackupOperation) do(ctx context.Context, r *Backup, c *Client) error {
	...
	mask := strings.Join([]string{"description", "labels"}, ",")
...
func (c *Client) backupDiffsForRawDesired(ctx context.Context, rawDesired *Backup, opts ...dcl.ApplyOption) (initial, desired *Backup, diffs []backupDiff, err error) {
...
func diffBackup(c *Client, desired, actual *Backup, opts ...dcl.ApplyOption) ([]backupDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Name, actual.Name, dcl.Info{OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("Name")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToBackupDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func convertFieldDiffToBackupDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]backupDiff, error) {
	var diffs []backupDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := backupDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameTobackupApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameTobackupApiOperation(op string, opts ...dcl.ApplyOption) (backupApiOperation, error) {
	switch op {

	case "updateBackupUpdateBackupOperation":
		return &updateBackupUpdateBackupOperation{}, nil
