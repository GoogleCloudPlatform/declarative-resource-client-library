...
func caPoolGetURL(userBasePath string, r *CaPool) (string, error) {
	params := map[string]interface{}{
		"project":  dcl.ValueOrEmptyString(r.Project),
		"location": dcl.ValueOrEmptyString(r.Location),
		"name":     dcl.ValueOrEmptyString(r.Name),
	}
	return dcl.URL("projects/{{project}}/locations/{{location}}/caPools/{{name}}", "https://privateca.googleapis.com/v1/", userBasePath, params), nil
}

func caPoolListURL(userBasePath, project, location string) (string, error) {
	params := map[string]interface{}{
		"project":  project,
		"location": location,
	}
	return dcl.URL("projects/{{project}}/locations/{{location}}/caPools", "https://privateca.googleapis.com/v1/", userBasePath, params), nil

}

func caPoolCreateURL(userBasePath, project, location, name string) (string, error) {
	params := map[string]interface{}{
		"project":  project,
		"location": location,
		"name":     name,
	}
	return dcl.URL("projects/{{project}}/locations/{{location}}/caPools?caPoolId={{name}}", "https://privateca.googleapis.com/v1/", userBasePath, params), nil

}

func caPoolDeleteURL(userBasePath string, r *CaPool) (string, error) {
	params := map[string]interface{}{
		"project":  dcl.ValueOrEmptyString(r.Project),
		"location": dcl.ValueOrEmptyString(r.Location),
		"name":     dcl.ValueOrEmptyString(r.Name),
...
func (op *updateCaPoolUpdateCaPoolOperation) do(ctx context.Context, r *CaPool, c *Client) error {
	_, err := c.GetCaPool(ctx, r.URLNormalized())
	if err != nil {
		return err
	}
	...
	err = o.Wait(ctx, c.Config, "https://privateca.googleapis.com/v1/", "GET")
...
func (c *Client) listCaPoolRaw(ctx context.Context, project, location, pageToken string, pageSize int32) ([]byte, error) {
	u, err := caPoolListURL(c.Config.BasePath, project, location)
	if err != nil {
		return nil, err
	}
...
func (c *Client) listCaPool(ctx context.Context, project, location, pageToken string, pageSize int32) ([]*CaPool, string, error) {
	b, err := c.listCaPoolRaw(ctx, project, location, pageToken, pageSize)
	if err != nil {
		return nil, "", err
	}
...
func (op *deleteCaPoolOperation) do(ctx context.Context, r *CaPool, c *Client) error {
	r, err := c.GetCaPool(ctx, r.URLNormalized())
	if err != nil {
		if dcl.IsNotFound(err) {
			c.Config.Logger.Infof("CaPool not found, returning. Original error: %v", err)
			return nil
		}
		c.Config.Logger.Warningf("GetCaPool checking for existence. error: %v", err)
		return err
	}

	u, err := caPoolDeleteURL(c.Config.BasePath, r.URLNormalized())
	if err != nil {
		return err
	}
	...
	if err := o.Wait(ctx, c.Config, "https://privateca.googleapis.com/v1/", "GET"); err != nil {
		return err
	}
...
func (op *createCaPoolOperation) do(ctx context.Context, r *CaPool, c *Client) error {
	...
	u, err := caPoolCreateURL(c.Config.BasePath, project, location, name)
	...
	if _, err := c.GetCaPool(ctx, r.URLNormalized()); err != nil {
...
func (c *Client) getCaPoolRaw(ctx context.Context, r *CaPool) ([]byte, error) {
	...
	u, err := caPoolGetURL(c.Config.BasePath, r.URLNormalized())
	if err != nil {
		return nil, err
	}
	resp, err := dcl.SendRequest(ctx, c.Config, "GET", u, &bytes.Buffer{}, c.Config.RetryProvider)
...
func (c *Client) caPoolDiffsForRawDesired(ctx context.Context, rawDesired *CaPool, opts ...dcl.ApplyOption) (initial, desired *CaPool, diffs []*dcl.FieldDiff, err error) {
	...
	if sh := dcl.FetchStateHint(opts); sh != nil {
		if r, ok := sh.(*CaPool); !ok {
			c.Config.Logger.Warningf("Initial state hint was of the wrong type; expected CaPool, got %T", sh)
		} else {
			fetchState = r
		}
	}
	...
	rawInitial, err := c.GetCaPool(ctx, fetchState.URLNormalized())
	if rawInitial == nil {
		if !dcl.IsNotFound(err) {
			c.Config.Logger.Warningf("Failed to retrieve whether a CaPool resource already exists: %s", err)
			return nil, nil, nil, fmt.Errorf("failed to retrieve CaPool resource: %v", err)
		}
		c.Config.Logger.Info("Found that CaPool resource did not exist.")
		// Perform canonicalization to pick up defaults.
		desired, err = canonicalizeCaPoolDesiredState(rawDesired, rawInitial)
		return nil, desired, nil, err
	}
	c.Config.Logger.Infof("Found initial state for CaPool: %v", rawInitial)
	c.Config.Logger.Infof("Initial desired state for CaPool: %v", rawDesired)
	...
	c.Config.Logger.Infof("Canonicalized initial state for CaPool: %v", initial)
	...
	c.Config.Logger.Infof("Canonicalized desired state for CaPool: %v", desired)
...
func canonicalizeCaPoolIssuancePolicy(des, initial *CaPoolIssuancePolicy, opts ...dcl.ApplyOption) *CaPoolIssuancePolicy {
	...
	if dcl.IsZeroValue(des.AllowedKeyTypes) {
		des.AllowedKeyTypes = initial.AllowedKeyTypes
	} else {
		cDes.AllowedKeyTypes = des.AllowedKeyTypes
	}
...
func (r *CaPool) getFields() (string, string, string) {
	n := r.URLNormalized()
	return dcl.ValueOrEmptyString(n.Project), dcl.ValueOrEmptyString(n.Location), dcl.ValueOrEmptyString(n.Name)
}

func (r *CaPool) createFields() (string, string, string) {
	n := r.URLNormalized()
	return dcl.ValueOrEmptyString(n.Project), dcl.ValueOrEmptyString(n.Location), dcl.ValueOrEmptyString(n.Name)
}

func (r *CaPool) deleteFields() (string, string, string) {
	n := r.URLNormalized()
	return dcl.ValueOrEmptyString(n.Project), dcl.ValueOrEmptyString(n.Location), dcl.ValueOrEmptyString(n.Name)
}
...
func (r *CaPool) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "UpdateCaPool" {
		...
		return dcl.URL("projects/{{project}}/locations/{{location}}/caPools/{{name}}", "https://privateca.googleapis.com/v1/", userBasePath, fields), nil
