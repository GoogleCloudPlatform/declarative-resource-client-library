...
// can identify it.
func (r *AccessPolicy) Describe() dcl.ServiceTypeVersion {
...
func (l *AccessPolicyList) HasNext() bool {
	return l.nextToken != ""
}
...
func (l *AccessPolicyList) Next(ctx context.Context, c *Client) error {
	...
	items, token, err := c.listAccessPolicy(ctx, l.parent, l.nextToken, l.pageSize)
	if err != nil {
		return err
	}
	l.Items = items
...
func (c *Client) GetAccessPolicy(ctx context.Context, r *AccessPolicy) (*AccessPolicy, error) {
	...
	result, err := unmarshalAccessPolicy(b, c)
	if err != nil {
		return nil, err
	}
	...
	result, err = canonicalizeAccessPolicyNewState(c, result, r)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DeleteAllAccessPolicy(ctx context.Context, parent string, filter func(*AccessPolicy) bool) error {
	...
	err = c.deleteAllAccessPolicy(ctx, filter, listObj.Items)
	if err != nil {
		return err
	}
	for listObj.HasNext() {
...
func applyAccessPolicyHelper(c *Client, ctx context.Context, rawDesired *AccessPolicy, opts ...dcl.ApplyOption) (*AccessPolicy, error) {
	...
	initial, desired, fieldDiffs, err := c.accessPolicyDiffsForRawDesired(ctx, rawDesired, opts...)
	if err != nil {
	...
	var ops []accessPolicyApiOperation
	if create {
	...
	newDesired, err := canonicalizeAccessPolicyDesiredState(rawDesired, newState)
	if err != nil {
		return nil, err
	}
	c.Config.Logger.Infof("Diffing using canonicalized desired state: %v", newDesired)
	newDiffs, err := diffAccessPolicy(c, newDesired, newState)
	if err != nil {
		return nil, err
	}
