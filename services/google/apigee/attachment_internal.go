...
func (c *Client) attachmentDiffsForRawDesired(ctx context.Context, rawDesired *Attachment, opts ...dcl.ApplyOption) (initial, desired *Attachment, diffs []attachmentDiff, err error) {
...
func diffAttachment(c *Client, desired, actual *Attachment, opts ...dcl.ApplyOption) ([]attachmentDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Name, actual.Name, dcl.Info{Type: "ReferenceType", OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("Name")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToAttachmentDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func convertFieldDiffToAttachmentDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]attachmentDiff, error) {
	var diffs []attachmentDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := attachmentDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameToattachmentApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameToattachmentApiOperation(op string, opts ...dcl.ApplyOption) (attachmentApiOperation, error) {
	switch op {
