...
func endpointConfigSelectorGetURL(userBasePath string, r *EndpointConfigSelector) (string, error) {
	...
	return dcl.URL("projects/{{project}}/locations/{{location}}/endpointConfigSelectors/{{name}}", "https://networkservices.googleapis.com/v1beta1/", userBasePath, params), nil
}
...
func endpointConfigSelectorListURL(userBasePath, project, location string) (string, error) {
	...
	return dcl.URL("projects/{{project}}/locations/{{location}}/endpointConfigSelectors", "https://networkservices.googleapis.com/v1beta1/", userBasePath, params), nil
...
func endpointConfigSelectorCreateURL(userBasePath, project, location, name string) (string, error) {
	...
	return dcl.URL("projects/{{project}}/locations/{{location}}/endpointConfigSelectors?endpointConfigSelectorId={{name}}", "https://networkservices.googleapis.com/v1beta1/", userBasePath, params), nil
...
func diffEndpointConfigSelector(c *Client, desired, actual *EndpointConfigSelector, opts ...dcl.ApplyOption) ([]endpointConfigSelectorDiff, error) {
	...
	if ds, err := dcl.Diff(desired.Name, actual.Name, dcl.Info{OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("Name")); len(ds) != 0 || err != nil {
		...
		dsOld, err := convertFieldDiffToEndpointConfigSelectorDiff(ds, opts...)
		if err != nil {
			return nil, err
		}
...
func (r *EndpointConfigSelector) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "UpdateEndpointConfigSelector" {
		...
		return dcl.URL("projects/{{project}}/locations/{{location}}/endpointConfigSelectors/{{name}}", "https://networkservices.googleapis.com/v1beta1/", userBasePath, fields), nil
...
func convertFieldDiffToEndpointConfigSelectorDiff(fds []*dcl.FieldDiff, opts ...dcl.ApplyOption) ([]endpointConfigSelectorDiff, error) {
	var diffs []endpointConfigSelectorDiff
	for _, fd := range fds {
		for _, op := range fd.ResultingOperation {
			diff := endpointConfigSelectorDiff{Diffs: []*dcl.FieldDiff{fd}, FieldName: fd.FieldName}
			if op == "Recreate" {
				diff.RequiresRecreate = true
			} else {
				op, err := convertOpNameToendpointConfigSelectorApiOperation(op, opts...)
				if err != nil {
					return nil, err
				}
				diff.UpdateOp = op
...
func convertOpNameToendpointConfigSelectorApiOperation(op string, opts ...dcl.ApplyOption) (endpointConfigSelectorApiOperation, error) {
	switch op {

	case "updateEndpointConfigSelectorUpdateEndpointConfigSelectorOperation":
		return &updateEndpointConfigSelectorUpdateEndpointConfigSelectorOperation{}, nil
