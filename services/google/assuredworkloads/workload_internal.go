...
func workloadGetURL(userBasePath string, r *Workload) (string, error) {
	...
	return dcl.URL("organizations/{{organization}}/locations/{{location}}/workloads/{{name}}", "https://us-central1-assuredworkloads.googleapis.com/v1/", userBasePath, params), nil
}
...
func workloadListURL(userBasePath, organization, location string) (string, error) {
	...
	return dcl.URL("organizations/{{organization}}/locations/{{location}}/workloads", "https://us-central1-assuredworkloads.googleapis.com/v1/", userBasePath, params), nil
...
func (op *deleteWorkloadOperation) do(ctx context.Context, r *Workload, c *Client) error {
	...
	if err != nil {
		...
		c.Config.Logger.Warningf("GetWorkload checking for existence. error: %v", err)
		return err
	}

	err = r.workloadDeletePreAction(ctx, c)
	if err != nil {
		return err
	}
...
func (op *createWorkloadOperation) do(ctx context.Context, r *Workload, c *Client) error {
	...
	if err := o.Wait(ctx, c.Config, "https://us-central1-assuredworkloads.googleapis.com/v1/", "GET"); err != nil {
...
func (c *Client) workloadDiffsForRawDesired(ctx context.Context, rawDesired *Workload, opts ...dcl.ApplyOption) (initial, desired *Workload, diffs []workloadDiff, err error) {
	c.Config.Logger.Info("Fetching initial state...")
	// First, let us see if the user provided a state hint.  If they did, we will start fetching based on that.
	var fetchState *Workload
	if sh := dcl.FetchStateHint(opts); sh != nil {
		if r, ok := sh.(*Workload); !ok {
			c.Config.Logger.Warningf("Initial state hint was of the wrong type; expected Workload, got %T", sh)
		} else {
			fetchState = r
		}
	}
	...
	if rawInitial == nil {
		...
		c.Config.Logger.Info("Found that Workload resource did not exist.")
	...
	c.Config.Logger.Infof("Canonicalized initial state for Workload: %v", initial)
	...
	c.Config.Logger.Infof("Canonicalized desired state for Workload: %v", desired)
...
func diffWorkload(c *Client, desired, actual *Workload, opts ...dcl.ApplyOption) ([]workloadDiff, error) {
...
func (r *Workload) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "UpdateWorkload" {
		...
		return dcl.URL("organizations/{{organization}}/locations/{{location}}/workloads/{{name}}", "https://us-central1-assuredworkloads.googleapis.com/v1/", userBasePath, fields), nil
