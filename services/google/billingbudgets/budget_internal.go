...
func (r *BudgetAmount) validate() error {
	if err := dcl.ValidateAtMostOneOfFieldsSet([]string{"SpecifiedAmount", "LastPeriodAmount"}, r.SpecifiedAmount, r.LastPeriodAmount); err != nil {
		return err
	}
...
func budgetGetURL(userBasePath string, r *Budget) (string, error) {
	params := map[string]interface{}{
		"billingAccount": dcl.ValueOrEmptyString(r.BillingAccount),
		"name":           dcl.ValueOrEmptyString(r.Name),
	}
	return dcl.URL("billingAccounts/{{billingAccount}}/budgets/{{name}}", "https://billingbudgets.googleapis.com/v1/", userBasePath, params), nil
}

func budgetListURL(userBasePath, billingAccount string) (string, error) {
	params := map[string]interface{}{
		"billingAccount": billingAccount,
	}
	return dcl.URL("billingAccounts/{{billingAccount}}/budgets", "https://billingbudgets.googleapis.com/v1/", userBasePath, params), nil

}

func budgetCreateURL(userBasePath, billingAccount string) (string, error) {
	params := map[string]interface{}{
		"billingAccount": billingAccount,
	}
	return dcl.URL("billingAccounts/{{billingAccount}}/budgets", "https://billingbudgets.googleapis.com/v1/", userBasePath, params), nil
...
func budgetDeleteURL(userBasePath string, r *Budget) (string, error) {
	params := map[string]interface{}{
		"billingAccount": dcl.ValueOrEmptyString(r.BillingAccount),
		"name":           dcl.ValueOrEmptyString(r.Name),
	}
...
func newUpdateBudgetUpdateBudgetRequest(ctx context.Context, f *Budget, c *Client) (map[string]interface{}, error) {
	...
	b, err := c.getBudgetRaw(ctx, f.URLNormalized())
	if err != nil {
		return nil, err
	}
...
func (c *Client) listBudgetRaw(ctx context.Context, billingAccount, pageToken string, pageSize int32) ([]byte, error) {
	u, err := budgetListURL(c.Config.BasePath, billingAccount)
	if err != nil {
		return nil, err
	}
...
func (c *Client) listBudget(ctx context.Context, billingAccount, pageToken string, pageSize int32) ([]*Budget, string, error) {
	b, err := c.listBudgetRaw(ctx, billingAccount, pageToken, pageSize)
	if err != nil {
		return nil, "", err
	}
	...
	for _, v := range m.Budgets {
		...
		res.BillingAccount = &billingAccount
		l = append(l, res)
	}
...
func (op *deleteBudgetOperation) do(ctx context.Context, r *Budget, c *Client) error {
	r, err := c.GetBudget(ctx, r.URLNormalized())
	if err != nil {
		if dcl.IsNotFound(err) {
			c.Config.Logger.Infof("Budget not found, returning. Original error: %v", err)
			return nil
		}
		c.Config.Logger.Warningf("GetBudget checking for existence. error: %v", err)
		return err
	}

	u, err := budgetDeleteURL(c.Config.BasePath, r.URLNormalized())
	if err != nil {
		return err
	}
	...
	for i := 1; i <= maxRetry; i++ {
		_, err = c.GetBudget(ctx, r.URLNormalized())
		if !dcl.IsNotFound(err) {
			if i == maxRetry {
...
func (c *Client) getBudgetRaw(ctx context.Context, r *Budget) ([]byte, error) {
	...
	u, err := budgetGetURL(c.Config.BasePath, r.URLNormalized())
	if err != nil {
		return nil, err
	}
	resp, err := dcl.SendRequest(ctx, c.Config, "GET", u, &bytes.Buffer{}, c.Config.RetryProvider)
...
func (c *Client) budgetDiffsForRawDesired(ctx context.Context, rawDesired *Budget, opts ...dcl.ApplyOption) (initial, desired *Budget, diffs []*dcl.FieldDiff, err error) {
	...
	if sh := dcl.FetchStateHint(opts); sh != nil {
		if r, ok := sh.(*Budget); !ok {
			c.Config.Logger.Warningf("Initial state hint was of the wrong type; expected Budget, got %T", sh)
		} else {
			fetchState = r
		}
	}
	...
	rawInitial, err := c.GetBudget(ctx, fetchState.URLNormalized())
	if rawInitial == nil {
		if !dcl.IsNotFound(err) {
			c.Config.Logger.Warningf("Failed to retrieve whether a Budget resource already exists: %s", err)
			return nil, nil, nil, fmt.Errorf("failed to retrieve Budget resource: %v", err)
		}
		c.Config.Logger.Info("Found that Budget resource did not exist.")
		// Perform canonicalization to pick up defaults.
		desired, err = canonicalizeBudgetDesiredState(rawDesired, rawInitial)
		return nil, desired, nil, err
	}
	c.Config.Logger.Infof("Found initial state for Budget: %v", rawInitial)
	c.Config.Logger.Infof("Initial desired state for Budget: %v", rawDesired)
	...
	c.Config.Logger.Infof("Canonicalized initial state for Budget: %v", initial)
	...
	c.Config.Logger.Infof("Canonicalized desired state for Budget: %v", desired)
...
func canonicalizeBudgetDesiredState(rawDesired, rawInitial *Budget, opts ...dcl.ApplyOption) (*Budget, error) {
	...
	if dcl.IsZeroValue(rawDesired.ThresholdRules) {
		canonicalDesired.ThresholdRules = rawInitial.ThresholdRules
	} else {
		canonicalDesired.ThresholdRules = rawDesired.ThresholdRules
	}
...
func canonicalizeBudgetBudgetFilter(des, initial *BudgetBudgetFilter, opts ...dcl.ApplyOption) *BudgetBudgetFilter {
	...
	if dcl.IsZeroValue(des.Projects) {
		des.Projects = initial.Projects
	} else {
		cDes.Projects = des.Projects
	}
	if dcl.IsZeroValue(des.CreditTypes) {
		des.CreditTypes = initial.CreditTypes
	} else {
	...
	if dcl.IsZeroValue(des.Services) {
		des.Services = initial.Services
	} else {
		cDes.Services = des.Services
	}
	if dcl.IsZeroValue(des.Subaccounts) {
		des.Subaccounts = initial.Subaccounts
	} else {
...
func canonicalizeBudgetAllUpdatesRule(des, initial *BudgetAllUpdatesRule, opts ...dcl.ApplyOption) *BudgetAllUpdatesRule {
	...
	if dcl.IsZeroValue(des.MonitoringNotificationChannels) {
		des.MonitoringNotificationChannels = initial.MonitoringNotificationChannels
	} else {
...
func (r *Budget) getFields() (string, string) {
	n := r.URLNormalized()
	return dcl.ValueOrEmptyString(n.BillingAccount), dcl.ValueOrEmptyString(n.Name)
}

func (r *Budget) createFields() string {
	n := r.URLNormalized()
	return dcl.ValueOrEmptyString(n.BillingAccount)
}

func (r *Budget) deleteFields() (string, string) {
	n := r.URLNormalized()
	return dcl.ValueOrEmptyString(n.BillingAccount), dcl.ValueOrEmptyString(n.Name)
}
...
func (r *Budget) updateURL(userBasePath, updateName string) (string, error) {
	...
	if updateName == "UpdateBudget" {
		fields := map[string]interface{}{
			"billingAccount": dcl.ValueOrEmptyString(n.BillingAccount),
			"name":           dcl.ValueOrEmptyString(n.Name),
		}
		return dcl.URL("billingAccounts/{{billingAccount}}/budgets/{{name}}", "https://billingbudgets.googleapis.com/v1/", userBasePath, fields), nil
...
func expandBudget(c *Client, f *Budget) (map[string]interface{}, error) {
	...
	if v, err := dcl.DeriveField("billingAccounts/%s/budgets/%s", f.Name, f.BillingAccount, f.Name); err != nil {
...
func flattenBudget(c *Client, i interface{}) *Budget {
	...
	res.AllUpdatesRule = flattenBudgetAllUpdatesRule(c, m["notificationsRule"])
