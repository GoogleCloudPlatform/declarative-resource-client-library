...
func PatchDeploymentToUnstructured(r *dclService.PatchDeployment) *unstructured.Resource {
	...
	if r.PatchConfig != nil && r.PatchConfig != dclService.EmptyPatchDeploymentPatchConfig {
		...
		if r.PatchConfig.RetryStrategy != nil && r.PatchConfig.RetryStrategy != dclService.EmptyPatchDeploymentPatchConfigRetryStrategy {
			rPatchConfigRetryStrategy := make(map[string]interface{})
			if r.PatchConfig.RetryStrategy.Enabled != nil {
				rPatchConfigRetryStrategy["enabled"] = *r.PatchConfig.RetryStrategy.Enabled
			}
			rPatchConfig["retryStrategy"] = rPatchConfigRetryStrategy
		}
...
func UnstructuredToPatchDeployment(u *unstructured.Resource) (*dclService.PatchDeployment, error) {
	...
	if _, ok := u.Object["patchConfig"]; ok {
		if rPatchConfig, ok := u.Object["patchConfig"].(map[string]interface{}); ok {
			...
			if _, ok := rPatchConfig["retryStrategy"]; ok {
				if rPatchConfigRetryStrategy, ok := rPatchConfig["retryStrategy"].(map[string]interface{}); ok {
					r.PatchConfig.RetryStrategy = &dclService.PatchDeploymentPatchConfigRetryStrategy{}
					if _, ok := rPatchConfigRetryStrategy["enabled"]; ok {
						if b, ok := rPatchConfigRetryStrategy["enabled"].(bool); ok {
							r.PatchConfig.RetryStrategy.Enabled = dcl.Bool(b)
						} else {
							return nil, fmt.Errorf("r.PatchConfig.RetryStrategy.Enabled: expected bool")
						}
					}
				} else {
					return nil, fmt.Errorf("r.PatchConfig.RetryStrategy: expected map[string]interface{}")
				}
			}
