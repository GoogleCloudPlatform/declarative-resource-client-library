...
import (
	...
	dclService "github.com/GoogleCloudPlatform/declarative-resource-client-library/services/google/vertex/alpha"
...
func EndpointToUnstructured(r *dclService.Endpoint) *unstructured.Resource {
	...
	for _, rDeployedModelsVal := range r.DeployedModels {
		...
		if rDeployedModelsVal.EnableContainerLogging != nil {
			rDeployedModelsObject["enableContainerLogging"] = *rDeployedModelsVal.EnableContainerLogging
		}
		...
		if rDeployedModelsVal.SharedResources != nil {
			rDeployedModelsObject["sharedResources"] = *rDeployedModelsVal.SharedResources
		}
		...
		if s, ok := u.Object["deployedModels"].([]interface{}); ok {
			for _, o := range s {
				if objval, ok := o.(map[string]interface{}); ok {
					...
					if _, ok := objval["enableContainerLogging"]; ok {
						if b, ok := objval["enableContainerLogging"].(bool); ok {
							rDeployedModels.EnableContainerLogging = dcl.Bool(b)
						} else {
							return nil, fmt.Errorf("rDeployedModels.EnableContainerLogging: expected bool")
						}
					}
					...
					if _, ok := objval["sharedResources"]; ok {
						if s, ok := objval["sharedResources"].(string); ok {
							rDeployedModels.SharedResources = dcl.String(s)
						} else {
							return nil, fmt.Errorf("rDeployedModels.SharedResources: expected string")
						}
					}
